...
//
//   BERTH_DB=./demo.db bun run scripts/seed-demo.ts
//
// Then start the dev server with BERTH_DEMO=1 BERTH_DB=./demo.db
...
const dbPath = process.env.BERTH_DB ?? resolve(process.cwd(), 'demo.db');
...
console.log(`Run: BERTH_DEMO=1 BERTH_DB=${dbPath} bun run dev`);
sqlite.close();
