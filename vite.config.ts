...
// Add more via BERTH_ALLOWED_HOSTS="foo.local,.example.com" (comma-separated;
// leading dot matches any subdomain).
const extraHosts = (process.env.BERTH_ALLOWED_HOSTS ?? '')
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean);
...
export default defineConfig({
  plugins: [sveltekit()],
  server: {
    port: Number(process.env.BERTH_PORT ?? 5202),
    strictPort: true,
    host: process.env.BERTH_HOST ?? '127.0.0.1',
