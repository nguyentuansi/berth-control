...
// Add more via HARBORCTL_ALLOWED_HOSTS="foo.local,.example.com" (comma-separated;
// leading dot matches any subdomain).
const extraHosts = (process.env.HARBORCTL_ALLOWED_HOSTS ?? '')
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean);
...
export default defineConfig({
  plugins: [sveltekit()],
  server: {
    port: Number(process.env.HARBORCTL_PORT ?? 5202),
    strictPort: true,
    host: process.env.HARBORCTL_HOST ?? '127.0.0.1',
