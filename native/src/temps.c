// Apple Silicon SoC temperature reader.
//
// Reads CPU/GPU/SoC die temperatures from the private IOHIDEventSystem
// thermal sensors. No sudo, no SMC-key maintenance — IOHIDEventSystem
// gives stable per-chip sensor names that the JS wrapper layer in
// index.ts groups into cpu/gpu/soc readings.
//
...
// Renamed mt_* → bt_* to avoid collision with the upstream package if a
// dependent ever pulls both in. See LICENSE-THIRD-PARTY.md.
...
int bt_read_snapshot(bt_snapshot_t* out) {
  ...
  bt_sensor_t* sensors = (bt_sensor_t*)calloc(cap, sizeof(bt_sensor_t));
  if (!sensors) {
    CFRelease(services);
    CFRelease((CFTypeRef)client);
    return 6;
  ...
  for (CFIndex i = 0; i < n; i++) {
    ...
    // plausibility filter — drop ghost reads that fall outside any
    // physically reasonable die temperature
  ...
  bt_sensor_t* shrunk = (bt_sensor_t*)realloc(sensors, used * sizeof(bt_sensor_t));
